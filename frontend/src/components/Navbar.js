      ...
      .then((res) =>
        res.data
          .map((user) => ({
            label: (
              <div
                className="navbar-load-options-container"
                onClick={this.navigateToUserPage.bind(this, user._id)}
              >
                <div>
                  <img
                    className="navbar-load-options-profilePic"
                    src={user.profilePic}
                    alt=""
                  />
                </div>
                <div className="navbar-load-options-username-and-fullName-div">
                  <p className="navbar-load-options-username">
                    {user.username}
                  </p>
                  <p className="navbar-load-options-fullName">
                    {user.fullName}
                  </p>
                </div>
              </div>
          ...
          .filter((user) =>
            user.value.toLowerCase().includes(inputText.toLowerCase())
          )
      );
          ...
          <ul>
            <li>
              <Link to="/homepage" className="navbar-home-icon-link">
                <i
                  className="fa fa-home navbar-home-icon"
                  aria-hidden="true"
                ></i>
              </Link>
            </li>
