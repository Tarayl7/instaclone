...
class ProfilePage extends React.Component {
    ...
    componentDidMount = async () => {
        ...
        const loggedInUserPosts = this.state.posts.filter(post => post.username === this.props.loggedInUserUsername);
    ...
    openFollowersModal = async () => {
        ...
        const toggleFollowersArray = this.props.loggedInUserFollowersArray.map(user => {
    ...
    openFollowingModal = async () => {
        ...
        const toggleFollowingArray = this.props.loggedInUserFollowingArray.map(user => {
            user.display = 'inline';
            user.buttonText = 'Following';
            user.buttonBackgroundColor = 'white';
            user.buttonTextColor = 'black';
    ...
    render() {
        ...
        const loggedInUserPosts = this.props.loggedInUserPosts.map((post, index) => {
            return(
                <div key={index}>
                    {
                        post.type === 'image' ?
                        <img className='profile-page-post-image' src={post.url} alt='' onClick={this.openPostModal.bind(this, post._id)} /> :
                        post.type === 'video' &&
                        <video className='profile-page-post-video' src={post.url} onClick={this.openPostModal.bind(this, post._id)} />
                    }
                </div>
            )
        })
            ...
            <div>
                ...
                <div className='profile-page-container'>
                    <div className='profile-page-top'>
                        <div>
                            <img className='profile-page-profile-pic' src={this.props.loggedInUserProfilePic} alt=''></img>
                        </div>
                        <div>
                            <div className='profile-page-username-and-button-div'>
                                <div>
                                    <p className='profile-page-username'>{this.props.loggedInUserUsername}</p>
                                </div>
                                <div className='profile-page-button-div'>
                                    <button className='profile-page-edit-button' onClick={() => navigate('/edit')}>Edit Profile</button>
                                    <button className='profile-page-logout-button' onClick={this.logout}>Logout</button>
                                </div>
                            </div>
                            <div className='profile-page-posts-followers-and-following-div'>
                                <p className='profile-page-posts'><span className='profile-page-posts-span'>{this.props.loggedInUserPosts.length}</span> posts</p>
                                <p className='profile-page-followers' onClick={this.openFollowersModal}><span className='profile-page-followers-span'>{this.props.loggedInUserFollowersArray.length}</span> followers</p>
                                <p className='profile-page-following' onClick={this.openFollowingModal}><span className='profile-page-following-span'>{this.props.loggedInUserFollowingArray.length}</span> following</p>
                            </div>
                            <div className='profile-page-full-name-div'>
                                <p className='profile-page-full-name'>{this.props.loggedInUserFullName}</p>
                            </div>
                    ...
                    <div className='profile-page-middle'>
                        <div className='profile-page-file-upload-div'>
                            <input id='file' type='file' name='file' onChange={this.changeFile}></input>
                        ...
                        <form className='profile-page-form' onSubmit={this.handleSubmit}>
                            <div>
                                <img className='profile-page-preview-image' src='' alt='' ref={this.imageRef} style={{ display: this.state.imageDisplay }} />
                                <video className='profile-page-preview-video' src='' ref={this.videoRef} style={{ display: this.state.videoDisplay }} />
                            </div>
                            <div>
                                <textarea className='profile-page-textarea' type='text' value={this.state.caption} onChange={this.onChange} style={{ display: this.state.display }} />
                            </div>
                            <div>
                                <button className='profile-page-share-button' type='submit' disabled={this.state.disabled} style={{ display: this.state.display }}>Share</button>
                            </div>
                        </form>
                        <div className='profile-page-uploading-display' style={{ display: this.state.uploadingDisplay }}>Uploading video...</div>
                    </div>
                    <div className='profile-page-posts-div'>
                        {loggedInUserPosts}
                    </div>
                </div>
