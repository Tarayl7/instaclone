...
class Homepage extends React.Component {
    ...
    componentDidMount = async () => {
        ...
        const homepagePosts = this.state.posts.filter(post => post.username === this.props.loggedInUserUsername || usernamesOfFollowingArray.includes(post.username));
    ...
    changeLikes = (postId, likes) => {
        ...
        const usernamesOfPostLikes = likes.map(user => { return user.username })

        // If logged in user is in likes array, then delete user
        if (usernamesOfPostLikes.includes(this.props.loggedInUserUsername)) {
            axios.put('posts/delete_like/' + postId, { id: this.props.loggedInUserId })
