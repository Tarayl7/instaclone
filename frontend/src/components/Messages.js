...
class Messages extends React.Component {
  ...
  componentDidMount = () => {
    ...
    axios
      .get("user/" + this.props.loggedInUserId, {
        headers: {
    ...
    if (this.state.isUpdated) {
      ...
      axios
        ...
        .then((res) => {
          this.setState({
            messages: res.data.messages,
            displayedMessages: res.data.messages
              .filter(
                (user) =>
                  user.recipientUsername === this.state.recipientUsername
              )
              .map((message) => message.messagesBetweenRecipient)[0],
            isUpdated: false,
          });
        });
    }
  ...
  displayMessages = (recipientId, recipientProfilePic, recipientUsername) => {
    ...
    const displayedMessages = this.state.messages
      .filter((user) => user.recipientUsername === recipientUsername)
      .map((message) => message.messagesBetweenRecipient)[0];
  ...
  handleSubmit = async (e) => {
    ...
    if (this.state.inputValue !== "") {
      ...
      await axios
        .get("user/" + this.state.recipientId, {
          headers: {
            "x-auth-token": this.props.token,
          },
        })
      ...
      const usernamesOfRecipientMessages = this.state.recipientMessages.map(
        (user) => {
      ...
      if (
        !usernamesOfRecipientMessages.includes(this.props.loggedInUserUsername)
      ) {
  ...
  render() {
    ...
    const displayedMessages = this.state.displayedMessages.map(
      (user, index) => {
        return (
          <div key={index}>
        ...
        <div className="messages-container">
          <div className="messages-left-box">
            <div className="messages-left-top-box">
              <p className="messages-username">
                {this.props.loggedInUserUsername}
              </p>
              <button
                className="messages-select-user"
                onClick={this.openMessagesModal}
              >
                <i className="fa fa-pencil" aria-hidden="true"></i>
              </button>
            </div>
            <div className="messages-recipients">{recipients}</div>
          </div>
          {this.state.recipientUsername === "" ? (
            ...
            <div className="messages-right-box">
              <div className="messages-right-top-box">
                <img
                  className="messages-recipient-profile-pic"
                  src={this.state.recipientProfilePic}
                  alt=""
                />
                <p className="messages-recipient-username">
                  {this.state.recipientUsername}
                </p>
              </div>
