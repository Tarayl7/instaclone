...
class PostModal extends React.Component {
    ...
    changeLikes = () => {
        ...
        if (usernamesOfPostLikes.includes(this.props.loggedInUserUsername)) {
            axios.put('posts/delete_like/' + this.props.postId, { id: this.props.loggedInUserId })
    ...
    render() {
        ...
        const usernamesOfPostLikes = this.props.postLikes.map(like => like.username);

        return (
            <Modal isOpen={this.props.postModalIsOpen} onRequestClose={this.props.closePostModal} style={{
                overlay: {
                    ...
                    <div className='post-modal-left-box'>
                        {
                            this.props.postType === 'image' ?
                                <img className='post-modal-image' src={this.props.postUrl} alt='' /> :
                                this.props.postType === 'video' &&
                                <video className='post-modal-video' src={this.props.postUrl} controls />
                        }
                    </div>
                    <div className='post-modal-right-box'>
                        <div className='post-modal-top-box-div'>
                            <div className='post-modal-profile-pic-and-usename-div'>
                                <img className='post-modal-profile-pic' src={this.props.postProfilePic} alt='' onClick={this.navigateToUserPage.bind(this, this.props.postUserId)} />
                                <p className='post-modal-username' onClick={this.navigateToUserPage.bind(this, this.props.postUserId)}>{this.props.postUsername}</p>
                            </div>
                            <div>
                                <i className='fa fa-ellipsis-h post-modal-ellipsis-icon' onClick={this.openEllipsisModal}></i>
                            </div>
                        </div>
                        ...
                        <div className='post-modal-icon-and-likes-div'>
                            <div className='post-modal-icons'>
                                {
                                    (usernamesOfPostLikes.includes(this.props.loggedInUserUsername)) ?
                                        <i className="fa fa-heart post-modal-heart-icon-truthy" onClick={this.changeLikes.bind(this)}></i> :
                                        <i className="fa fa-heart post-modal-heart-icon-falsy" onClick={this.changeLikes.bind(this)}></i>
                                }
                                <i className="fa fa-comment post-modal-comment-icon" onClick={() => this.inputRef.current.focus()}></i>
                                <i className="fa fa-envelope post-modal-envelope-icon" onClick={() => navigate('/messages')}></i>
                            </div>
                            {
                                this.props.postLikes.length === 1 ?
                                    <div className='post-modal-likes'>
                                        <p>Liked by <span className='post-modal-first-span'>{this.props.postLikes[0].username}</span></p>
                                    </div> :
                                    this.props.postLikes.length > 1 &&
                                    <div className='post-modal-likes'>
                                        <p>Liked by <span className='post-modal-first-span' onClick={this.navigateToUserPage.bind(this, this.props.postLikes[0].id)}>{this.props.postLikes[0].username}</span> and <span className='post-modal-second-span' onClick={this.openLikesModal}>others</span></p>
                                    </div>
                            }
                        </div>
                        <div>
                            <form className='post-modal-form' onSubmit={this.handleSubmit}>
                                <div>
                                    <input className='post-modal-input' value={this.state.inputValue} ref={this.inputRef} onChange={this.onChange} />
                                </div>
                                <div>
